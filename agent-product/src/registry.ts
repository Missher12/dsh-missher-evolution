import { observeCheck, type VerificationEntry, type VerificationPlan } from './verification.js'
import type {
  ErrorKind,
  ExperienceCapsule,
  ExperienceScope,
  Outcome,
  PreferenceId,
  TaskType,
  TrialAssignment,
  WorkflowStep,
} from './types.js'

const RULE_ID = /^rule_[a-z0-9_]{1,96}$/u
const MAX_WORKFLOW_STEPS = 12
const MAX_INJECTED_RULES = 4

export interface OpenTurnInput {
  taskHash: string
  taskType: TaskType
  correction: boolean
  preference: PreferenceId | null
  experience?: ExperienceCapsule
}

export interface TurnSnapshot extends OpenTurnInput {
  sessionId: string
  turnId: string
  workflowSteps: WorkflowStep[]
  injectedRuleIds: string[]
  errorKind: ErrorKind
  assistantOutcome: Outcome | null
  experience?: ExperienceCapsule
  successfulToolCount: number
  failedToolCount: number
  experimentAssignments: TrialAssignment[]
  captured: boolean
  filtered: boolean
  touchedAt: number
  verificationEntries?: VerificationEntry[]
  verificationActiveKeys?: string[]
  exitCodes?: (number | null)[]
}

export interface TurnRegistryOptions {
  maxEntries?: number
  ttlMs?: number
  now?: () => number
}

function identity(value: unknown, label: string): string {
  const normalized = typeof value === 'number' && Number.isSafeInteger(value)
    ? String(value)
    : value
  if (
    typeof normalized !== 'string'
    || normalized.length === 0
    || normalized.length > 512
    || /[\u0000-\u001f\u007f]/u.test(normalized)
  ) {
    throw new TypeError(`${label}_invalid`)
  }
  return normalized
}

function keyFor(sessionId: string, turnId: string): string {
  return `${sessionId.length}:${sessionId}${turnId}`
}

function verificationKey(plan: VerificationPlan): string {
  return `${plan.ruleId}:${plan.instructionHash}:${plan.constraintId}`
}

function copyTurn(turn: TurnSnapshot): TurnSnapshot {
  return {
    ...turn,
    ...(turn.experience === undefined ? {} : { experience: copyExperience(turn.experience) }),
    workflowSteps: [...turn.workflowSteps],
    injectedRuleIds: [...turn.injectedRuleIds],
    experimentAssignments: turn.experimentAssignments.map(assignment => ({ ...assignment })),
    ...(turn.verificationEntries === undefined ? {} : { verificationEntries: structuredClone(turn.verificationEntries
      .filter(entry => turn.verificationActiveKeys?.includes(verificationKey(entry)))) }),
    ...(turn.verificationActiveKeys === undefined ? {} : { verificationActiveKeys: [...turn.verificationActiveKeys] }),
    ...(turn.exitCodes === undefined ? {} : { exitCodes: [...turn.exitCodes] }),
  }
}

function copyExperience(experience: ExperienceCapsule): ExperienceCapsule {
  return {
    ...experience,
    scope: { ...experience.scope },
    intentIds: [...experience.intentIds],
    constraintIds: [...experience.constraintIds],
    verificationIds: [...experience.verificationIds],
    ...(experience.topicIds === undefined ? {} : { topicIds: [...experience.topicIds] }),
    ...(experience.excludedConstraintIds === undefined ? {} : { excludedConstraintIds: [...experience.excludedConstraintIds] }),
  }
}

export class TurnRegistry {
  private readonly turns = new Map<string, TurnSnapshot>()
  private readonly subagents = new Map<string, number>()
  private readonly maxEntries: number
  private readonly ttlMs: number
  private readonly now: () => number

  constructor(options: TurnRegistryOptions = {}) {
    const maxEntries = options.maxEntries ?? 1_000
    const ttlMs = options.ttlMs ?? 60 * 60 * 1_000
    if (!Number.isInteger(maxEntries) || maxEntries < 1) throw new RangeError('max_entries_invalid')
    if (!Number.isFinite(ttlMs) || ttlMs <= 0) throw new RangeError('ttl_invalid')
    this.maxEntries = maxEntries
    this.ttlMs = ttlMs
    this.now = options.now ?? Date.now
  }

  open(sessionId: unknown, turnId: unknown, input: OpenTurnInput): TurnSnapshot | undefined {
    const session = identity(sessionId, 'session')
    const turn = identity(turnId, 'turn')
    const now = this.now()
    this.cleanupAt(now)
    const key = keyFor(session, turn)
    const existing = this.turns.get(key)
    if (existing !== undefined) {
      existing.taskHash = input.taskHash
      existing.taskType = input.taskType
      existing.correction ||= input.correction
      existing.preference = input.preference ?? existing.preference
      if (input.experience !== undefined) existing.experience = copyExperience(input.experience)
      existing.touchedAt = now
      return copyTurn(existing)
    }
    if (!this.makeRoom()) return undefined
    const opened: TurnSnapshot = {
      sessionId: session,
      turnId: turn,
      taskHash: input.taskHash,
      taskType: input.taskType,
      correction: input.correction,
      preference: input.preference,
      workflowSteps: [],
      injectedRuleIds: [],
      errorKind: 'none',
      assistantOutcome: null,
      ...(input.experience === undefined ? {} : { experience: copyExperience(input.experience) }),
      successfulToolCount: 0,
      failedToolCount: 0,
      experimentAssignments: [],
      captured: false,
      filtered: false,
      touchedAt: now,
    }
    this.turns.set(key, opened)
    return copyTurn(opened)
  }

  setInjectedRules(sessionId: unknown, turnId: unknown, ruleIds: readonly string[]): boolean {
    if (
      !Array.isArray(ruleIds)
      || ruleIds.length > MAX_INJECTED_RULES
      || new Set(ruleIds).size !== ruleIds.length
      || ruleIds.some(id => typeof id !== 'string' || !RULE_ID.test(id))
    ) return false
    const state = this.find(sessionId, turnId)
    if (state === undefined || state.captured || state.filtered) return false
    state.injectedRuleIds = [...ruleIds]
    state.touchedAt = this.now()
    return true
  }

  setExperimentAssignments(
    sessionId: unknown,
    turnId: unknown,
    assignments: readonly TrialAssignment[],
  ): boolean {
    if (
      !Array.isArray(assignments)
      || assignments.length > 1
      || assignments.some(assignment =>
        assignment === null
        || typeof assignment !== 'object'
        || typeof assignment.ruleId !== 'string'
        || !RULE_ID.test(assignment.ruleId)
        || (assignment.arm !== 'treatment' && assignment.arm !== 'control'))
    ) return false
    const state = this.find(sessionId, turnId)
    if (state === undefined || state.captured || state.filtered) return false
    state.experimentAssignments = assignments.map(assignment => ({ ...assignment }))
    state.touchedAt = this.now()
    return true
  }

  observeTool(
    sessionId: unknown,
    turnId: unknown,
    step: WorkflowStep,
    errorKind: ErrorKind = 'none',
  ): boolean {
    const state = this.find(sessionId, turnId)
    if (state === undefined || state.captured || state.filtered) return false
    const previous = state.workflowSteps.at(-1)
    if (previous !== step) {
      if (state.workflowSteps.length < MAX_WORKFLOW_STEPS) state.workflowSteps.push(step)
      else state.workflowSteps[MAX_WORKFLOW_STEPS - 1] = 'other'
    }
    if (errorKind !== 'none') state.errorKind = errorKind
    if (errorKind === 'none') state.successfulToolCount = Math.min(1_000, state.successfulToolCount + 1)
    else state.failedToolCount = Math.min(1_000, state.failedToolCount + 1)
    state.touchedAt = this.now()
    return true
  }

  observeAssistant(sessionId: unknown, turnId: unknown, outcome: Outcome): boolean {
    const state = this.find(sessionId, turnId)
    if (state === undefined || state.captured || state.filtered) return false
    state.assistantOutcome = outcome
    state.touchedAt = this.now()
    return true
  }

  observeError(sessionId: unknown, turnId: unknown, errorKind: ErrorKind): boolean {
    const state = this.find(sessionId, turnId)
    if (state === undefined || state.captured || state.filtered) return false
    state.errorKind = errorKind === 'none' ? 'unknown' : errorKind
    state.assistantOutcome = 'failure'
    state.touchedAt = this.now()
    return true
  }

  markFiltered(sessionId: unknown, turnId: unknown): boolean {
    const state = this.find(sessionId, turnId)
    if (state === undefined || state.captured || state.filtered) return false
    state.filtered = true
    state.touchedAt = this.now()
    return true
  }

  claimCapture(sessionId: unknown, turnId: unknown): TurnSnapshot | undefined {
    const state = this.find(sessionId, turnId)
    if (state === undefined || state.captured || state.filtered) return undefined
    state.captured = true
    state.touchedAt = this.now()
    return copyTurn(state)
  }

  setVerificationPlans(sessionId: unknown, turnId: unknown, plans: readonly VerificationPlan[],
    scopes?: ReadonlyMap<string, ExperienceScope>): boolean {
    const state = this.find(sessionId, turnId)
    if (!state || !this.canSetVerificationPlans(sessionId, turnId, plans)) return false
    const prior = state.verificationEntries ?? []
    const active = plans.filter(plan => state.injectedRuleIds.includes(plan.ruleId))
    for (const plan of active) if (!prior.some(entry => verificationKey(entry) === verificationKey(plan))) {
      const scope = scopes?.get(plan.ruleId)
      prior.push({ ...plan, ...(scope === undefined ? {} : { scope: { ...scope } }),
        observations: [], invalidated: false, limitReached: false })
    }
    state.verificationEntries = prior
    state.verificationActiveKeys = active.map(verificationKey)
    return true
  }

  canSetVerificationPlans(sessionId: unknown, turnId: unknown, plans: readonly VerificationPlan[]): boolean {
    const state = this.find(sessionId, turnId)
    if (!state || state.captured || state.filtered || plans.length > 24) return false
    const keys = new Set((state.verificationEntries ?? []).map(verificationKey))
    for (const plan of plans) keys.add(verificationKey(plan))
    return keys.size <= 24
  }

  observeVerification(sessionId: unknown, turnId: unknown, report: unknown): boolean {
    const state = this.find(sessionId, turnId)
    if (!state || state.captured || state.filtered) return false
    return observeCheck((state.verificationEntries ?? [])
      .filter(entry => state.verificationActiveKeys?.includes(verificationKey(entry))), report)
  }

  observeToolEvidence(sessionId: unknown, turnId: unknown, exitCode: number | null | undefined, readOnly: boolean): void {
    const state = this.find(sessionId, turnId)
    if (!state || state.captured || state.filtered) return
    if (!readOnly) for (const entry of state.verificationEntries ?? []) {
      if (entry.checkerId !== 'exit-status-v1' && entry.observations.length) entry.invalidated = true
    }
    if (exitCode !== undefined) {
      state.exitCodes ??= []
      if (state.exitCodes.length < 1_000) state.exitCodes.push(exitCode)
      else state.exitCodes[state.exitCodes.length - 1] = null
    }
  }

  markSubagent(sessionId: unknown): void {
    this.subagents.set(identity(sessionId, 'session'), this.now())
  }

  isSubagent(sessionId: unknown): boolean {
    const session = identity(sessionId, 'session')
    const now = this.now()
    this.cleanupAt(now)
    if (!this.subagents.has(session)) return false
    this.subagents.set(session, now)
    return true
  }

  discardSession(sessionId: unknown): number {
    const session = identity(sessionId, 'session')
    let removed = 0
    for (const [key, state] of this.turns) {
      if (state.sessionId !== session) continue
      this.turns.delete(key)
      removed += 1
    }
    this.subagents.delete(session)
    return removed
  }

  cleanup(): number {
    return this.cleanupAt(this.now())
  }

  get size(): number {
    this.cleanup()
    return this.turns.size
  }

  private find(sessionId: unknown, turnId: unknown): TurnSnapshot | undefined {
    const session = identity(sessionId, 'session')
    const turn = identity(turnId, 'turn')
    this.cleanupAt(this.now())
    return this.turns.get(keyFor(session, turn))
  }

  private cleanupAt(now: number): number {
    const cutoff = now - this.ttlMs
    let removed = 0
    for (const [key, state] of this.turns) {
      if (state.touchedAt > cutoff) continue
      this.turns.delete(key)
      removed += 1
    }
    for (const [session, touchedAt] of this.subagents) {
      if (touchedAt > cutoff) continue
      this.subagents.delete(session)
      removed += 1
    }
    return removed
  }

  private makeRoom(): boolean {
    if (this.turns.size < this.maxEntries) return true
    let eviction: [string, TurnSnapshot] | undefined
    for (const entry of this.turns) {
      const state = entry[1]
      if (!state.captured && !state.filtered) continue
      if (eviction === undefined || state.touchedAt < eviction[1].touchedAt) eviction = entry
    }
    if (eviction === undefined) return false
    this.turns.delete(eviction[0])
    return this.turns.size < this.maxEntries
  }
}
