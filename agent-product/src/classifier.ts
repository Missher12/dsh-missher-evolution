...
const CORRECTION_PATTERNS: readonly RegExp[] = [
  /(?:上次|刚才|之前).{0,40}(?:错了|不对|弄错|出错)|(?:不要|不能|别)再/iu,
...
export function classifyPrompt(value: unknown): PromptClassification {
  ...
  let taskType = TASK_RULES.find(([pattern]) => pattern.test(text))?.[1] ?? 'general'
  // A local data repair is not software development solely because it says 修复.
  if (taskType === 'coding' && /(?:采集|收集|整理|核对).{0,8}数据/u.test(text)
    && /发布时间|原始日期|空值|未知值/u.test(text)
    && !/python|javascript|typescript|代码|测试|开发|程序|脚本|接口|\bapi\b|refactor|debug|implement/iu.test(text)) {
    taskType = 'data'
  }
